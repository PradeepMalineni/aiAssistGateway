...
def _score_with_rules(traits: dict) -> dict:
    ...
    for gw, w in weights.get("exposure", {}).get(public, {}).items():
        scores[gw]=scores.get(gw,0)+w
    for gw, w in weights.get("data_classification", {}).get(cat, {}).items():
        scores[gw]=scores.get(gw,0)+w

    p99 = traits["summary"].get("max_body_bytes_hint", 0)/1024
    key = ">256" if p99>256 else "<=256"
    for gw, w in weights.get("payload.p99_kb", {}).get(key, {}).items():
        scores[gw]=scores.get(gw,0)+w
...
def decide(traits: dict, model=None) -> dict:
    ...
    # LLM hook could be added here; for now return rules-only in offline mode
