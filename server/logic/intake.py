...
def discover_oas(workspace_root: str, globs_pattern=None):
    ...
    matches = []
    for pat in patterns:
        matches += [str(p) for p in root.glob(pat)]
...
def load_oas(path: str):
    ...
    try:
        validate(data, BASIC_OAS_SCHEMA)
    except ValidationError as e:
        raise ValueError(f"Basic OpenAPI shape invalid: {e.message}")
    return data
...
def _scan_schemas_for_pii(components):
    ...
    for name, schema in schemas.items():
        ...
        for prop in props:
            if PII_HINTS.search(prop):
                hints.append({"schema": name, "field": prop})
    return hints
...
def extract_traits(spec: dict):
        ...
        for m, op in ops.items():
            if m.lower() in {"get","post","put","delete","patch","options","head"}:
                ...
                for mt, media in content.items():
                    ex = media.get("example") or (media.get("examples", {}).get("default", {})
                                                  if isinstance(media.get("examples"), dict) else None)
                    if isinstance(ex, (str, bytes)):
                        max_body_bytes = max(max_body_bytes, len(str(ex).encode()))
    ...
    traits = {
        "summary": {
            ...
            "max_body_bytes_hint": max_body_bytes,
        },
        "data_signals": {
            "pii_fields": pii_hits,
            "pci_suspected": has_pci
        },
    }
    return traits
