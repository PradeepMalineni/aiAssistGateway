...
def extract_traits(spec: dict, sidecar: dict|None=None):
    ...
    traits={'summary': {'endpoints': len(paths), 'operations': methods, 'public': public_exposure, 'max_body_bytes_hint': max_body_bytes},
            'data_signals': {'pii_fields': pii_hits, 'pci_suspected': has_pci}}
    if sidecar:
        if 'exposure' in sidecar: traits['summary']['public'] = (sidecar['exposure']=='public')
        rate = sidecar.get('traffic_profile',{}).get('suggested_rate_per_app_per_min')
        if rate: traits.setdefault('thresholds',{})['rate_limit_per_app_per_min']=int(rate)
        pci = sidecar.get('data_classification',{}).get('pci')
        pii = sidecar.get('data_classification',{}).get('pii')
        if pci is not None: traits['data_signals']['pci_suspected']=bool(pci)
        if pii is not None and pii and not traits['data_signals']['pii_fields']:
            traits['data_signals']['pii_fields']=[{'schema':'*','field':'pii'}]
    return traits
