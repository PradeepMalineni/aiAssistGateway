...
def generate_bundle(spec: dict, plan: dict, out_dir: str, decision: dict):
    ...
    (out/'compliance_report.md').write_text('# Compliance Report\n\n- Required controls: ' + ', '.join(plan['required_controls']) + f"\n- Decision: {decision['gateway']}"); artifacts.append(str(out/'compliance_report.md'))
    if decision['gateway']=='apigee':
        ...
        dp_tpl=env.get_template('datapower/config/config.xml.j2'); (out/'datapower/config.xml').write_text(dp_tpl.render(name='AI-Generated-Service')); artifacts.append(str(out/'datapower/config.xml'))
